...
var theWorkerDude = function (options) {
    var deferred = q.defer();

    var worker = new Worker(function (event) {
        //////////////////////////
        // Include all of JASON //
        //////////////////////////
        ...
        function inject(inj) {
            for (var key in inj) {
                // hope to see key = undefined
                // console.log('key = ' + global.key + inj[key]);
                global[key] = inj[key];
            }
        }

        function loadLibs(libs, reqwire) {
            try {
                for (var key in libs) {
                    global[key] = reqwire(libs[key]);
                }
                console.log(_, libs[key]);
            } catch(e) {
                console.log(e.stack);
            }
        }
        ...
        onmessage = function (event) {
            var data = event.data;
            switch (data.action) {
                case 'inject': inject(JASON.parse(data.inject)); break;
                case 'require': loadLibs(JASON.parse(data.libs), JASON.parse(data.require)); break;
                case 'execute': execute(data.args, JASON.parse(data.fn)); break;
    ...
    worker.onmessage = function (event) {
        if (event.data.error)
            return deferred.reject(event.data.error);
    ...
    if(options.inject) {
        worker.postMessage({ 
            action: 'inject', 
            inject: JASON.stringify(options.inject) 
        });
    }

    // NYI (super hard, apparently)
    // if(options.require) {
    //     worker.postMessage({ 
    //         action: 'require', 
    //         libs: JASON.stringify(options.require), 
    //         require: JASON.stringify(require) 
    //     });
    // }
...
// theWorkerDude.spread

module.exports = theWorkerDude;